...
class LoginController {
  ...
  static async getProfile(req: Request, res: Response): Promise<Response> {
    try {
      // Get user ID from the authenticated request (from auth middleware)
  ...
  static async updateProfile(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const { firstName, lastName, middleName, username, email, role, name } = req.body;
      ...
      const updatedUser = await LoginModel.findByIdAndUpdate(userId, updateData, {
        new: true,
        runValidators: true,
      });
