...
class FarmerController {
  // Create new farmer
  static async createFarmer(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
      ...
      // Log the audit
      await auditLog.create('farmer', result.insertedId.toString(), farmerData, req);
      ...
      // Log the failure
      await auditLog.failure('CREATE', 'farmer', undefined, error as Error, req);
  ...
  static async getFarmerCountByBarangay(req: Request, res: Response): Promise<Response> {
    try {
  ...
  // Get all farmers
  static async getAllFarmers(_req: Request, res: Response): Promise<Response> {
    try {
  ...
  // Update farmer by ID
  static async updateFarmer(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
      const { id } = req.params;
      ...
      const { firstName, middleName, lastName, crop, area, barangay, contact, fullName } =
        req.body as FarmerRequestBody;
      ...
      // Log the failure
      await auditLog.failure('UPDATE', 'farmer', req.params.id, error as Error, req);
  ...
  // Delete farmer by ID
  static async deleteFarmer(req: AuthenticatedRequest, res: Response): Promise<Response> {
    try {
      const { id } = req.params;
      console.log('Farmer ID:', id);
      ...
      // Log the failure
      await auditLog.failure('DELETE', 'farmer', req.params.id, error as Error, req);
